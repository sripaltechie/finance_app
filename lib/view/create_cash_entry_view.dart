...
class CashEntryView extends StatefulWidget {
  @override
  _CashEntryViewState createState() => _CashEntryViewState();
}
...
class _CashEntryViewState extends State<CashEntryView> {
  ...
  Widget build(BuildContext context) {
    final drcrViewModel = Provider.of<DrcrViewModel>(context);
            ...
            child:
                  ...
                  builder: (context, value, _) {
                    switch (value.customersList.status!) {
                      ...
                      case Status.ERROR:
                        return Center(
                            child:
                                Text(value.customersList.message.toString()));
                      case Status.COMPLETED:
                        return TypeAheadFormField(
                          textFieldConfiguration: TextFieldConfiguration(
                            ...
                            onSubmitted: (_) {
                              Utils.fieldFocusChange(context,
                                  _customerFocusNode, _creditFocusNode);
                            },
                          ...
                          suggestionsCallback: (pattern) async {
                            return value.customersList.data!.customers!.where(
                                (item) => item.fullname
                                    .toString()
                                    .toLowerCase()
                          ...
                          itemBuilder: (context, itemData) {
                            return ListTile(
                              title: Text(itemData.fullname.toString()),
                            );
                          },
                          onSuggestionSelected: (selectedItemData) {
                            setState(() {
                              _customerNameController.text =
                                  selectedItemData.fullname.toString();
                              _selectedCustomerId = selectedItemData.id;
                ...
                children: [
                    ...
                    child: TextFormField(
                      ...
                      onFieldSubmitted: (_) {
                        Utils.fieldFocusChange(
                            context, _creditFocusNode, _debitFocusNode);
                      },
                    ),
                  ),
                  SizedBox(width: 16.0),
                  ...
                  Expanded(
                    child: TextFormField(
                      ...
                      onFieldSubmitted: (_) {
                        Utils.fieldFocusChange(
                            context, _debitFocusNode, _noteFocusNode);
                      },
                    ),
                  ),
                ],
                ...
                child: Consumer<YesnoViewModel>(
                  builder: (context, value, _) {
                    switch (value.yesno.status!) {
                      ...
                      case Status.ERROR:
                        return Center(
                            child: Text(value.yesno.message.toString()));
                      case Status.COMPLETED:
                        return Column(
                          children: [
                            Row(
              ...
              DropdownButtonFormField<int>(
                ...
                items: _creditExp.map((creditExp) {
                  return DropdownMenuItem<int>(
                    value: int.tryParse(creditExp['id'].toString()) ?? null,
              ...
              //     create: (BuildContext context) => paymentmodesListViewModel,
              ...
              //             return Center(
              //                 child: Text(
              //                     pmvalue.PaymentmodesList.message.toString()));
              //           case Status.COMPLETED:
                  ...
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      ...
                      if (_paymentmode == 50) {
                        int _amount = 0;
                        _amount =
                            (int.parse(_creditController.text.toString()) > 0)
                                ? int.parse(_creditController.text.toString())
                                : int.parse(_debitController.text.toString());
                        if (_pmtrans.length >= 2) {
                          int ttl = 0;
                          _pmtrans.forEach((e) {
                            if (e.paymentmode == 1 && _isCreditAmt) {
                              _collectedby = true;
                            }
                            if (_isCreditAmt) {
                            ...
                            } else if (int.tryParse(e.debit.toString()) !=
                                    null &&
                                int.tryParse(e.debit.toString())! > 0) {
                              ttl += int.parse(e.debit.toString());
                            } else {
                              Utils.flushBarErrorMessage(
                                  "Multi payment amounts not entered properly...!!",
                                  context);
                      ...
                      } else if (_paymentmode == 1 && _isCreditAmt) {
                        _collectedby = true;
                      }
                      if (_dateController.text.contains(' ')) {
                        ...
                        _dateController.text =
                            Utils.userFormat(_dateController.text);
                      }
                      Map drcr = {
                        "credit": _creditController.text.toString(),
                        "customer": _selectedCustomerId.toString(),
                        "date":
                            Utils.sqlFormat(_dateController.text.toString()),
                        "debit": _debitController.text.toString(),
                        "forint": _forIntValue.toString(),
                        "note": _noteController.text.toString(),
                        "collectedby":
                            (_collectedby) ? _user.collector.toString() : "0",
                        "paymentmode": _paymentmode.toString(),
                        "pmtrans": (_paymentmode == 50)
                            ? jsonEncode(_pmtrans)
                            : jsonEncode([]),
                        "creditexp": _creditExpValue.toString(),
                        "showdaybook": _showDaybook.toString(),
                      };

                      drcrViewModel.createDrcrApi(drcr, context);
                      bool condition = false;
                      while (!condition) {
                        await Future.delayed(Duration(seconds: 1));
                        // Replace the condition with your own logic
                        if (drcrViewModel.iscreateDrcrCalled) {
                          if (int.tryParse(drcrViewModel.createDrcr.data
                                      .toString()) !=
                                  null ||
                              drcrViewModel.createDrcr.Apistatus != null) {
                            emptyFields();
                            drcrViewModel.setiscreateDrcrCalled(false);
                            condition = true;
                          }
                        }
                      }
